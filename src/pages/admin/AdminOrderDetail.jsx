...
import { 
  ArrowLeft, Loader2, AlertCircle, Package, User, MapPin, CreditCard, Clock
} from 'lucide-react';
    ...
    return (
      <AdminLayout title="Detalhes do Pedido">
        <div className="bg-red-50 text-red-600 p-4 rounded-lg flex items-center gap-2">
          <AlertCircle size={20} /> {error || "Pedido não encontrado"}
        </div>
        <div className="mt-4">
  ...
  return (
    ...
    >
      <div className="mb-6 flex justify-between items-center">
        <Link to="/admin/orders" className="inline-flex items-center text-gray-600 hover:text-primary">
            ...
            <table className="w-full text-left">
              <thead className="bg-gray-50 border-b">
                <tr>
                  ...
                  <th className="px-6 py-3 text-sm font-medium text-gray-500 text-right">Qtd</th>
                  <th className="px-6 py-3 text-sm font-medium text-gray-500 text-right">Unitário</th>
                  <th className="px-6 py-3 text-sm font-medium text-gray-500 text-right">Total</th>
                </tr>
              </thead>
                ...
                {order.items.map((item) => (
                  <tr key={item.id}>
                    <td className="px-6 py-4">
                      ...
                      {/* Can add product image here if available in item data or via relation */}
                    </td>
              ...
              <tfoot className="bg-gray-50">
                <tr>
                  <td colSpan="3" className="px-6 py-3 text-right font-medium text-gray-500">Subtotal</td>
                  <td className="px-6 py-3 text-right font-bold text-gray-900">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(order.subtotal)}
                  </td>
                </tr>
                <tr>
                  <td colSpan="3" className="px-6 py-3 text-right font-medium text-gray-500">Frete</td>
                  <td className="px-6 py-3 text-right font-bold text-gray-900">
                    {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(order.shippingCost)}
                  </td>
                </tr>
                <tr className="bg-gray-100">
                  <td colSpan="3" className="px-6 py-4 text-right font-black text-gray-900 text-lg">Total</td>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-lg shadow p-6">
            ...
            <div className="space-y-4">
              <div>
                <p className="text-xs text-gray-500 uppercase mb-1">Status do Pedido</p>
              ...
              <div>
                ...
                <p className="font-medium capitalize">{order.paymentMethod || 'Não informado'}</p>
              </div>
              <div>
                <p className="text-xs text-gray-500 uppercase mb-1">Status Pagamento</p>
                <p className="font-medium">{order.paymentStatus || 'Pendente'}</p>
              </div>
            </div>
          </div>
        </div>
