...
const AdminMessages: React.FC = () => {
  ...
  const bulkActionMutation = useMutation({
    mutationFn: ({
      ...
      action: string;
      value?: any;
    }) => adminService.bulkAction(ids, action, value),
    onSuccess: () => {
      toast.success("Ação realizada com sucesso!");
      setSelectedIds([]);
      queryClient.invalidateQueries({ queryKey: ["admin-messages"] });
      // Also invalidate dashboard stats if status changed
  ...
  const handleBulkAction = (action: string, value?: any) => {
    ...
    bulkActionMutation.mutate({ ids: selectedIds, action, value });
  };

  const handleExport = async () => {
    try {
      const url = await adminService.exportMessages({
          ...
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between animate-in fade-in slide-in-from-top-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                className="border-blue-200 text-gray-700 hover:bg-gray-50"
              >
                Arquivar
              </Button>
              <Button
                    ...
                    >
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                        <Link
                          ...
                          className="text-indigo-600 hover:text-indigo-900 inline-flex items-center gap-1 px-3 py-1 border border-indigo-200 rounded-md hover:bg-indigo-50 transition-colors"
                        >
