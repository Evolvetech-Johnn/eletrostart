...
export class ImportExportService {
   ...
   */
  async parseImportFile(buffer: Buffer, mimetype: string): Promise<ProductImportData[]> {
    ...
    const hasSku = Object.values(headers).some(h => h.includes('sku') || h.includes('código'));
    if (!hasSku) {
